...
* **[Presentación del taller](https://view.genially.com/69de8ca748ebb6b0cef431de):** — *Aquí encontrarás un resumen de los conceptos clave.*
...
4. **¡Paso Crítico!** En la ventana de configuración, busca el desplegable **"Política de direcciones MAC"** y asegúrate de seleccionar: **"Generar nuevas direcciones MAC para todos los adaptadores de red"**. *(Si no haces esto, chocarás con la IP de tus compañeros).*
...
## Fase 2: Construcción del clúster
...
1. Para identificar mejor el rol de cada máquina en le clúster, cambiaremos el **hostname**. A continuación, mostramos una sugerencia de nombres, así como los comandos para configurarlos.
    ...
    ```
    > Nota: Después de esto, la terminal seguirá mostrando el nombre antiguo hasta que cierres sesión y vuelvas a entrar, o reinicies la máquina.
...
3. En el nodo que conformará el plano de control, ejecuta el siguiente comando para obtener la IP, el puerto y un token para unir al clúster un nodo.
    ```bash
    microk8s add-node
    ```
...
4. En los nodos trabajadores ejecuta el siguiente comando con la IP, puerto y token que has obtenido en el paso anterior.
    ```bash
    microk8s join <IP>:<PUERTO>/<TOKEN> --worker
    ```
...
```
---
## Comandos interactuar con el clúster
```bash
kubectl apply -f <nombre-fichero>
```
Para mandar un manifiesto YAML al plano de control y que éste se encargue de crear los objetos o realizar las operaciones necesarias.

```bash
kubectl api-resources
```
Para listar todos los objetos de Kubernetes.

```bash
kubectl delete <tipo-objeto-kubernetes> <nombre-objeto-kubernetes>
```
Para eliminar un objeto creado.

```bash
kubectl delete -f <nombre-fichero>
```
...
```
Muestra información sobre el estado de un pod, como el nodo en el que se está ejecutando, errores en su despliegue, etc.

```bash
kubectl logs -f <nombre-pod>
```
Para mostrar los logs de un pod determinado.
